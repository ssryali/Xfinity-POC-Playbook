...
import { requestBPAFNudge } from "@/lib/writer";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await requestBPAFNudge(strategicInput);
    return NextResponse.json(result);
  } catch (error) {
    console.error("[ANALYZE-BPAF] Error:", error instanceof Error ? error.message : error);
