...
import { requestTSNudge } from "@/lib/writer";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await requestTSNudge(strategicInput);
    return NextResponse.json({
      ...result,
      agentId: "US-CORE-16",
      agentName: "Thought-Starters, Not Content Generation",
    });
  } catch (error) {
    console.error("[ANALYZE-TS] Error:", error instanceof Error ? error.message : error);
    return NextResponse.json(
