...
import { requestDNNudge } from "@/lib/writer";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await requestDNNudge(strategicInput);
    return NextResponse.json(result);
  } catch (error) {
    console.error("[ANALYZE-DN] Error:", error instanceof Error ? error.message : error);
