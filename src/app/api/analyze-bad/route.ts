...
import { requestBADNudge } from "@/lib/writer";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await requestBADNudge(strategicInput);
    return NextResponse.json(result);
  } catch (error) {
    console.error("[ANALYZE-BAD] Error:", error instanceof Error ? error.message : error);
