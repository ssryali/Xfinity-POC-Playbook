...
import { requestRTBNudge } from "@/lib/writer";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await requestRTBNudge(strategicInput);
    return NextResponse.json(result);
  } catch (error) {
    console.error("[ANALYZE-RTB] Error:", error instanceof Error ? error.message : error);
