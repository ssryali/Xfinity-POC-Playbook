...
import { requestBCANudge } from "@/lib/writer";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await requestBCANudge(strategicInput);
    return NextResponse.json(result);
  } catch (error) {
    console.error("[ANALYZE-BCA] Error:", error instanceof Error ? error.message : error);
